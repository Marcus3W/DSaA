...
print(diagonalSum(A), "- Diagonal should be equal to 3")
...
def IsTriangularMatrix(A):
    for i in range(0, len(A)):
        for j in range(i + 1, len(A)):  # use i + 1 when you want to look at the next column
            if A[j][i] != 0:  # checks BELOW the diagonal line
                print("NO")
                return False
    print("YES")
    return True
...
def IsReverseTriangular(A):
    for i in range(0, len(A)):
        for j in range(i + 1, len(A)):
            if A[i][j] != 0:  # checks ABOVE the diagonal line
                print("No")
                return False
    print("Yes")
    return True
...
def evenSelectionSort(A):
    for i in range(0, len(A)):
        ...
        for j in range(i + 1, len(A)):  # looks at the next element in the list
...
def PrintEdges(A, B, C) :
    ...
    for i in range(0, len(B)):
        ...
        for j in range(0, len(C)):
            ...
            # this is using the 'numbers' in the subsets to determine which indexes to check
            if A[ B[i] ] [ C[j] ] == 1 or A[ C[j] ][ B[i] ] == 1:
